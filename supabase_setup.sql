...
CREATE POLICY "用户可以查看自己的心情日记" ON public.mood_entries
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "用户可以插入自己的心情日记" ON public.mood_entries
    FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY "用户可以更新自己的心情日记" ON public.mood_entries
    FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "用户可以删除自己的心情日记" ON public.mood_entries
    FOR DELETE USING (auth.uid() = user_id);
