...
console.log('[Workplace] script.js loaded v1778000010');
...
async function ensureAccountingHydratedFromCloud() {
    try {
        const raw = localStorage.getItem('accountingData');
        const isEmpty = !raw || raw === 'null' || raw === 'undefined' || raw === '{}' || raw === '[]' || raw === '{"records":[]}';
        if (!isEmpty) return;

        if (!window.auth || !window.auth.isAuthenticated || !window.auth.isAuthenticated()) return;
        if (!window.dataSync || !window.dataSync.isReady || typeof window.dataSync.syncData !== 'function') return;

        console.log('[AccountingHydrate] 本地记账为空，执行云端补拉');
        await window.dataSync.syncData('accountingData', 'cloud');

        if (typeof initAccounting === 'function') initAccounting();
        if (typeof postAccountingDataToStatsIframe === 'function') postAccountingDataToStatsIframe();
        console.log('[AccountingHydrate] 记账数据补拉完成');
    } catch (e) {
        console.warn('[AccountingHydrate] 记账补拉失败:', e);
    }
}

function ensureAccountingRenderedFromLocal() {
    try {
        const raw = localStorage.getItem('accountingData');
        const hasLocalData = raw && raw !== 'null' && raw !== 'undefined' && raw !== '{}' && raw !== '[]' && raw !== '{"records":[]}';
        if (!hasLocalData) return;

        const parsed = JSON.parse(raw);
        const localCount = Array.isArray(parsed?.records) ? parsed.records.length : 0;
        const memoryCount = (typeof accountingData !== 'undefined' && Array.isArray(accountingData?.records)) ? accountingData.records.length : 0;

        if (localCount > 0 && memoryCount === 0) {
            console.log('[AccountingRenderFix] 本地有数据但内存为空，触发重渲染:', localCount);
            if (typeof initAccounting === 'function') initAccounting();
            if (typeof postAccountingDataToStatsIframe === 'function') postAccountingDataToStatsIframe();
        }
    } catch (e) {
        console.warn('[AccountingRenderFix] 重渲染检查失败:', e);
    }
}
function initDate() {
...
// ==================== 同步失败重试（待办/健康/心情/记账通用）====================
const OFFLINE_PENDING_PREFIX = 'offline_pending_';
const SYNC_FAIL_MAP_KEY = 'sync_failures_v1';

function getOfflinePendingTypes() {
    const types = ['todos', 'healthRecords', 'moodEntries', 'accountingData'];
    return types.filter(t => {
        const pendingFlag = localStorage.getItem(OFFLINE_PENDING_PREFIX + t) === '1';
        return pendingFlag;
    });
}

function clearOfflinePendingTypes(types) {
    (types || []).forEach(t => localStorage.removeItem(OFFLINE_PENDING_PREFIX + t));
    updateSyncPendingBanner();
}

function setOfflinePending(type) {
    localStorage.setItem(OFFLINE_PENDING_PREFIX + type, '1');
    updateSyncPendingBanner();
}

function getSyncFailMap() {
    try {
        const raw = localStorage.getItem(SYNC_FAIL_MAP_KEY);
        if (!raw) return {};
        const parsed = JSON.parse(raw);
...
function setSyncFailMap(map) {
    localStorage.setItem(SYNC_FAIL_MAP_KEY, JSON.stringify(map));
    updateSyncPendingBanner();
}

function isSyncFailed(dataType, itemKey) {
    const map = getSyncFailMap();
    return Boolean(map?.[dataType]?.[itemKey]);
}

/** 仅重绘与同步失败标记相关的模块（dataType 不传则全部刷新） */
function renderSyncFailedUI(dataType) {
    try {
        if (!dataType || dataType === 'todos') {
            if (typeof renderTodos === 'function') renderTodos();
        }
        if (!dataType || dataType === 'healthRecords') {
            if (typeof updateHealthUI === 'function') updateHealthUI();
        }
        if (!dataType || dataType === 'moodEntries') {
            if (typeof renderMoodRecent === 'function') renderMoodRecent();
        }
        if (!dataType || dataType === 'accountingData') {
            if (typeof renderRecentRecords === 'function') renderRecentRecords();
        }
    } catch (e) {
        console.warn('[renderSyncFailedUI]', e);
    }
}

function markSyncFailed(dataType, itemKey, reason = '', { silent = false } = {}) {
    const map = getSyncFailMap();
    if (!map[dataType]) map[dataType] = {};
    map[dataType][itemKey] = { reason, at: Date.now() };
    setSyncFailMap(map);
    if (!silent) showToast('数据同步云端失败');
    // silent:true 场景也要立即刷新，确保失败状态能及时反馈到列表/UI
    renderSyncFailedUI(dataType);
}

function clearSyncFailed(dataType, itemKey) {
    const map = getSyncFailMap();
    if (!map?.[dataType]) return;
    delete map[dataType][itemKey];
    if (Object.keys(map[dataType]).length === 0) {
        delete map[dataType];
    }
    setSyncFailMap(map);
    renderSyncFailedUI(dataType);
...
/** 某类数据整表上传成功后，清除该类型下所有失败标记 */
function clearAllSyncFailedForType(dataType) {
    const map = getSyncFailMap();
    if (!map?.[dataType]) return;
    delete map[dataType];
    setSyncFailMap(map);
...
let syncPendingBannerDismissed = false;
let lastSyncPendingFailCount = 0;

function getSyncFailedCount() {
    const map = getSyncFailMap();
    return Object.values(map).reduce((sum, group) => {
        if (!group || typeof group !== 'object') return sum;
        return sum + Object.keys(group).length;
    }, 0);
}

function updateSyncPendingBanner() {
    const banner = document.getElementById('sync-pending-banner');
    const text = document.getElementById('sync-pending-banner-text');
    if (!banner || !text) return;

    const failCount = getSyncFailedCount();
    const pendingTypes = getOfflinePendingTypes();
    const pendingTypeCount = pendingTypes.length;
    if (failCount === 0 && pendingTypeCount === 0) {
        syncPendingBannerDismissed = false;
        lastSyncPendingFailCount = 0;
        banner.classList.add('hidden');
        return;
    }

    const totalCount = failCount + pendingTypeCount;
    if (totalCount !== lastSyncPendingFailCount) {
        syncPendingBannerDismissed = false;
        lastSyncPendingFailCount = totalCount;
    }

    if (syncPendingBannerDismissed) {
        return;
    }

    if (failCount > 0 && pendingTypeCount > 0) {
        text.textContent = `有 ${failCount} 条失败记录，另有 ${pendingTypeCount} 类离线数据待同步`;
    } else if (failCount > 0) {
        text.textContent = `有 ${failCount} 条数据尚未上传云端`;
    } else {
        text.textContent = `有 ${pendingTypeCount} 类离线数据待同步`;
    }
    banner.classList.remove('hidden');
}

function dismissSyncPendingBanner() {
    syncPendingBannerDismissed = true;
    const banner = document.getElementById('sync-pending-banner');
...
async function retryAllFailedSync() {
    const map = getSyncFailMap();
    const failedTypes = Object.keys(map || {});
    const pendingTypes = getOfflinePendingTypes();
    const types = [...new Set([...failedTypes, ...pendingTypes])];
    if (!types.length) {
        showToast('当前没有待重试数据');
        return;
    }
    ...
    for (const dataType of types) {
        try {
            await window.dataSync.uploadData(dataType);
            clearAllSyncFailedForType(dataType);
            clearOfflinePendingTypes([dataType]);
            successCount++;
        } catch (e) {
            failedCount++;
            console.warn('[retryAllFailedSync]', dataType, e);
        }
    }

    if (failedCount === 0) {
        showToast(`重试成功（${successCount}类）`);
    } else {
        showToast(`部分重试成功（成功${successCount}类，失败${failedCount}类）`);
    }
}
...
async function retrySyncData(dataType, itemKey = null) {
    if (!window.dataSync || !window.dataSync.isReady) {
        showToast('同步引擎未就绪，请稍后重试');
        return;
    }
    ...
    try {
        // 失败重试：只上传本地，避免下载覆盖；成功则清除该类型全部失败标记（整包已上传）
        await window.dataSync.uploadData(dataType);
        clearAllSyncFailedForType(dataType);
        clearOfflinePendingTypes([dataType]);
        ...
        if (itemKey) markSyncFailed(dataType, itemKey, e?.message || 'upload failed', { silent: true });
        showToastSyncError();
    }
}

// ==================== 离线同步策略选择 ====================
let __syncStrategyResolver = null;

function strategyToMergePolicy(strategy) {
...
function chooseSyncStrategy(strategy) {
    const resolver = __syncStrategyResolver;
    __syncStrategyResolver = null;

    const modal = document.getElementById('sync-strategy-modal');
    if (modal) modal.classList.add('hidden');

    if (resolver) resolver(strategy);
}

function closeSyncStrategyModal() {
    chooseSyncStrategy('merge');
}

function showSyncStrategyModal(pendingTypes) {
    const modal = document.getElementById('sync-strategy-modal');
    const msg = document.getElementById('sync-strategy-message');
    if (msg) {
        const readable = (pendingTypes || []).map(t => {
            if (t === 'todos') return '待办';
            if (t === 'healthRecords') return '健康打卡';
            if (t === 'moodEntries') return '心情日记';
            if (t === 'accountingData') return '记账';
            return t;
        }).join('、');
        msg.textContent = `检测到离线数据：${readable}。登录后如何同步？`;
    }

    if (modal) modal.classList.remove('hidden');

    return new Promise(resolve => {
        __syncStrategyResolver = resolve;
    });
}

// 统一处理同步失败事件：给当前本地数据打上“重试失败”的标记并提示
window.addEventListener('syncError', (event) => {
    try {
        const errors = event?.detail?.errors || [];
        if (!errors.length) return;

        // 避免一口气刷很多 toast：只提示一次
        showToastSyncError();

        errors.forEach(({ dataType }) => {
            // 给该数据类型的本地条目打标（用于渲染重试按钮）
            if (dataType === 'todos') {
                const raw = localStorage.getItem('todos');
                const arr = raw ? JSON.parse(raw) : [];
                if (Array.isArray(arr)) {
                    arr.forEach(t => {
                        if (t && t.id) markSyncFailed('todos', String(t.id), 'sync failed', { silent: true });
                    });
                }
                if (typeof renderTodos === 'function') renderTodos();
            }
            ...
            if (dataType === 'healthRecords') {
                ...
                if (obj && typeof obj === 'object' && !Array.isArray(obj)) {
                    Object.keys(obj).forEach(dateKey => {
                        ...
                        ['water', 'toilet', 'exercise'].forEach(field => {
                            const itemKey = `${dateKey}:${field}`;
                            markSyncFailed('healthRecords', itemKey, 'sync failed', { silent: true });
                        });
                    });
                }
                if (typeof updateHealthUI === 'function') updateHealthUI();
            ...
            if (dataType === 'moodEntries') {
                const raw = localStorage.getItem('moodEntries');
                const arr = raw ? JSON.parse(raw) : [];
                if (Array.isArray(arr)) {
                    arr.forEach(e => {
                            if (e && e.id) markSyncFailed('moodEntries', String(e.id), 'sync failed', { silent: true });
                    });
                }
                if (typeof renderMoodRecent === 'function') renderMoodRecent();
            }
            ...
            if (dataType === 'accountingData') {
                const raw = localStorage.getItem('accountingData');
                const obj = raw ? JSON.parse(raw) : { records: [] };
                const records = obj?.records && Array.isArray(obj.records) ? obj.records : [];
                records.forEach(r => {
                    if (r && r.id) markSyncFailed('accountingData', String(r.id), 'sync failed', { silent: true });
                });
                if (typeof renderRecentRecords === 'function') renderRecentRecords();
            }
        });
    } catch (e) {
        console.error('[SyncError handler] failed:', e);
    }
});

// 同步成功：仅清理本次成功类型的失败标记，避免误清其他未同步类型
window.addEventListener('syncComplete', (event) => {
    try {
        const successTypes = Array.isArray(event?.detail?.dataTypes) ? event.detail.dataTypes : [];
        if (!successTypes.length) return;
        successTypes.forEach(type => clearAllSyncFailedForType(type));
        clearOfflinePendingTypes(successTypes);
    } catch (e) {
        console.warn('[syncComplete] clear sync failures:', e);
    }
});

/** 认证失败或同步结束后，从 localStorage 重新灌入内存并渲染（离线刷新不丢界面） */
function refreshAllUserDataFromStorage() {
    try {
        ...
        console.error('[refreshAllUserDataFromStorage]', e);
    }
}
...
function toggleHealth(type) {
    try {
        ...
        const failKey = `${today}:${type}`;
        // 已登录：尝试直接上传；失败则 toast + 重试按钮
        if (window.dataSync && window.dataSync.isReady && window.auth && window.auth.isAuthenticated && window.auth.isAuthenticated()) {
            console.log('[Health] Uploading health records...');
            window.dataSync.uploadData('healthRecords')
                .then(() => {
                    clearSyncFailed('healthRecords', failKey);
                })
                .catch(err => {
                    markSyncFailed('healthRecords', failKey, err?.message || 'upload failed');
                    updateHealthUI();
                });
        } else {
            // 未登录：标记离线待同步，登录后由弹窗决定同步策略
            setOfflinePending('healthRecords');
        }
...
function saveHealth() {
    try {
        ...
        if (window.dataSync) {
            ...
            // 与同步引擎保持同样的时间戳命名，便于登录时判断“离线待同步”
            localStorage.setItem('healthRecords_timestamp', Date.now().toString());
        }
...
function saveTodos(changedTodoId = null) {
    // 保存到本地
    ...
    if (window.dataSync && window.dataSync.isReady) {
        ...
        window.dataSync.uploadData('todos').then(() => {
            ...
            if (changedTodoId !== null) clearSyncFailed('todos', String(changedTodoId));
            if (typeof renderTodos === 'function') renderTodos();
        }).catch(err => {
            console.error('[saveTodos] ❌ 上传失败:', err);
            if (changedTodoId !== null) markSyncFailed('todos', String(changedTodoId), err?.message || 'upload failed');
            if (typeof renderTodos === 'function') renderTodos();
        });
    } else {
        console.warn('[saveTodos] dataSync未就绪，跳过同步');
        // 没有明确登录状态也算离线：确保登录后能检测到待同步并弹窗
        if (!window.auth || (typeof window.auth.isAuthenticated === 'function' && !window.auth.isAuthenticated())) {
            setOfflinePending('todos');
        }
    }
}
...
function renderTodos() {
    ...
    dates.forEach(date => {
        ...
        groups[date].forEach(todo => {
            ...
            const todoActionsClass = 'todo-actions';
            li.innerHTML = `
...
function saveMood() {
    ...
    localStorage.setItem('moodEntries_timestamp', Date.now().toString());
    noteInput.value = '';
    renderMoodRecent();

    const entryId = newEntry.id;
    // 已登录：尝试上传；失败则 toast + 重试按钮
    if (window.dataSync && window.dataSync.isReady && window.auth && window.auth.isAuthenticated && window.auth.isAuthenticated()) {
        window.dataSync.uploadData('moodEntries')
            .then(() => {
                clearSyncFailed('moodEntries', String(entryId));
                renderMoodRecent();
            })
            .catch(err => {
                markSyncFailed('moodEntries', String(entryId), err?.message || 'upload failed');
                renderMoodRecent();
            });
    } else {
        // 未登录：标记离线待同步，登录后弹窗选择策略
        setOfflinePending('moodEntries');
    }
}
...
function renderMoodRecent() {
    ...
    recent.forEach(entry => {
        ...
        const moodActionsClass = 'mood-actions';
        div.innerHTML = `
...
function saveHealthEdit() {
    ...
    saveHealth();
    // 避免 syncAll 下载覆盖本地；编辑后直接上传
    if (window.dataSync && window.dataSync.isReady && window.auth && window.auth.isAuthenticated && window.auth.isAuthenticated()) {
        const fields = ['water', 'toilet', 'exercise'];
        window.dataSync.uploadData('healthRecords')
            .then(() => {
                // 清理该日三项的失败标记
                fields.forEach(field => clearSyncFailed('healthRecords', `${editingHealthDate}:${field}`));
                if (editingHealthDate === getTodayStr()) updateHealthUI();
            })
            .catch(err => {
                // 标记该日三项为失败（用于日视图/可重试）
                fields.forEach(field => markSyncFailed('healthRecords', `${editingHealthDate}:${field}`, err?.message || 'upload failed', { silent: true }));
                showToastSyncError();
                if (editingHealthDate === getTodayStr()) updateHealthUI();
            });
    } else {
...
function hydrateAccountingFromLocalIfNeeded() {
    try {
        const raw = localStorage.getItem('accountingData');
        if (!raw || raw === 'null' || raw === 'undefined' || raw === '{}' || raw === '[]' || raw === '{"records":[]}') {
            return false;
        }
        const parsed = JSON.parse(raw);
        const localRecords = Array.isArray(parsed?.records) ? parsed.records : [];
        const memoryRecords = Array.isArray(accountingData?.records) ? accountingData.records : [];

        if (localRecords.length > 0 && memoryRecords.length === 0) {
            accountingData = {
                ...(parsed && typeof parsed === 'object' ? parsed : {}),
                records: localRecords.map(normalizeAccountingRecord).filter(Boolean)
            };
            console.log('[AccountingHydrate] 内存为空，已从本地回灌:', accountingData.records.length);
            return true;
        }
    } catch (e) {
        console.warn('[AccountingHydrate] 本地回灌失败:', e);
    }
    return false;
}

// 统一解析记账日期，兼容历史数据中的多种日期格式（如 "YYYY-MM-DD HH:mm:ss"）
function parseAccountingDateSafe(dateValue) {
    ...
    // Safari 对 "YYYY-MM-DD HH:mm:ss" 兼容较差，优先转成 ISO 风格
    const normalized = text.includes(' ') ? text.replace(' ', 'T') : text;
    let parsed = new Date(normalized);
    if (!isNaN(parsed.getTime())) return parsed;

    // 兜底：把短横线日期转为斜杠，提升旧格式兼容性
    parsed = new Date(text.replace(/-/g, '/'));
    return parsed;
}

// 标准化记账类型，兼容历史数据中的中文类型值
function normalizeAccountingType(typeValue) {
...
function normalizeAccountingRecord(record) {
    if (!record || typeof record !== 'object') return null;
    const amountNum = Number(record.amount);
    return {
        ...record,
        type: normalizeAccountingType(record.type),
    ...
    if (saved) {
        ...
        if (Array.isArray(parsed)) {
            ...
            function extractAllRecords(data) {
                ...
                if (Array.isArray(data)) {
                    data.forEach(item => {
                        ...
                        } else if (item && typeof item === 'object' && (item.id || item.amount)) {
                            records.push(item);
                        }
                    });
                }
            ...
            allRecords.forEach((record, idx) => {
                const rid = record.id || ('legacy_' + idx + '_' + Date.now());
                if (!seenIds.has(rid)) {
                    seenIds.add(rid);
                    uniqueRecords.push({ ...record, id: rid });
                }
            });
            
            accountingData = { records: uniqueRecords.map(normalizeAccountingRecord).filter(Boolean) };
            // 保存修复后的数据
            saveAccountingData();
            console.log('[loadAccountingData] 已修复数据结构，去重后共', uniqueRecords.length, '项记录');
                ...
                accountingData = { records: parsed.map(normalizeAccountingRecord).filter(Boolean) };
            ...
            if (Array.isArray(parsed.records)) {
                ...
                accountingData = {
                    ...parsed,
                    records: parsed.records.map(normalizeAccountingRecord).filter(Boolean)
                };
            } else {
                // 错误的对象结构
                ...
                accountingData = { records: values.map(normalizeAccountingRecord).filter(Boolean) };
                console.log('[loadAccountingData] 已修复数据结构，共', accountingData.records.length, '项记录');
                // 保存修复后的结构
                saveAccountingData();
            }
...
// 保存记账数据
function saveAccountingData(changedRecordId = null) {
    // 保存到本地
    ...
    if (window.dataSync && window.dataSync.isReady) {
        ...
        window.dataSync.uploadData('accountingData').then(() => {
            ...
            if (changedRecordId !== null) clearSyncFailed('accountingData', String(changedRecordId));
            if (typeof renderRecentRecords === 'function') renderRecentRecords();
            postAccountingDataToStatsIframe();
        }).catch(err => {
            console.error('[saveAccountingData] ❌ 上传失败:', err);
            if (changedRecordId !== null) markSyncFailed('accountingData', String(changedRecordId), err?.message || 'upload failed');
            if (typeof renderRecentRecords === 'function') renderRecentRecords();
        });
    } else {
        console.warn('[saveAccountingData] dataSync未就绪，跳过同步');
        if (changedRecordId !== null && (!window.auth || (typeof window.auth.isAuthenticated === 'function' && !window.auth.isAuthenticated()))) {
            setOfflinePending('accountingData');
        }
    }

    // 无论在线离线都尝试刷新统计 iframe 数据源
    postAccountingDataToStatsIframe();
}
...
function updateAccountingSummary() {
    hydrateAccountingFromLocalIfNeeded();
    const today = new Date().toDateString();
    const todayRecords = accountingData.records.filter(r => 
        parseAccountingDateSafe(r.date).toDateString() === today
    );
    
    const todayExpense = todayRecords
        .filter(r => normalizeAccountingType(r.type) === 'expense')
...
function renderRecentRecords() {
    ...
    const recentRecords = accountingData.records
        .sort((a, b) => parseAccountingDateSafe(b.date) - parseAccountingDateSafe(a.date) || b.id - a.id)
        .slice(0, 6);
    ...
    container.innerHTML = recentRecords.map(record => {
        const normalizedType = normalizeAccountingType(record.type);
        const recordDate = parseAccountingDateSafe(record.date);
        ...
        return '<div class="accounting-record ' + normalizedType + '">' +
            ...
            '<span class="accounting-record-amount ' + normalizedType + '">' +
            (normalizedType === 'income' ? '+' : '-') + '¥' + Number(record.amount || 0).toFixed(2) +
            '</span>' +
...
function postAccountingDataToStatsIframe() {
    try {
        const iframe = document.getElementById('accounting-stats-iframe');
        const target = iframe?.contentWindow;
        if (!target) return false;

        const payload = {
            type: 'accounting-data-response',
            data: (accountingData && Array.isArray(accountingData.records)) ? accountingData.records : []
        };

        // 主动推送给 iframe，避免仅依赖 request 消息导致的时序丢包
        target.postMessage(payload, '*');
        console.log('[Main] 主动推送统计数据:', payload.data.length, '条');
        return true;
    } catch (e) {
        console.warn('[Main] 主动推送统计数据失败:', e);
        return false;
    }
}
...
function showAccountingStats() {
    ...
    // 打开弹窗后主动发送一次，避免 iframe 请求早于监听器或消息丢失
    setTimeout(() => postAccountingDataToStatsIframe(), 80);
}
...
// iframe 每次加载完成后主动再推送一次，提升首次打开稳定性
...
function renderAccountingStats(period) {
    ...
    const periodRecords = accountingData.records.filter(r => {
        const recordDate = parseAccountingDateSafe(r.date);
        return recordDate >= startDate && recordDate <= endDate;
    });
    
    // 计算总收入和总支出
    const totalIncome = periodRecords
        .filter(r => normalizeAccountingType(r.type) === 'income')
        .reduce((sum, r) => sum + r.amount, 0);
...
function renderCharts(records) {
    const expenseRecords = records.filter(r => normalizeAccountingType(r.type) === 'expense');
    const incomeRecords = records.filter(r => normalizeAccountingType(r.type) === 'income');
...
function renderCategoryList(records) {
    ...
    const categoryStats = {};
    records.forEach(r => {
        const normalizedType = normalizeAccountingType(r.type);
        if (!categoryStats[r.category]) {
        ...
        categoryStats[r.category].amount += Number(r.amount || 0);
...
function renderRecordsList(records) {
    ...
    const sortedRecords = [...records].sort((a, b) => 
        parseAccountingDateSafe(b.date) - parseAccountingDateSafe(a.date)
    );
    
    recordsList.innerHTML = sortedRecords.map(r => `
        <div class="stats-record-item">
            <div class="record-date">${parseAccountingDateSafe(r.date).toLocaleDateString('zh-CN')}</div>
            <div class="record-category">${r.category}</div>
            <div class="record-note">${r.note}</div>
            <div class="record-amount ${normalizeAccountingType(r.type)}">
                ${normalizeAccountingType(r.type) === 'income' ? '+' : '-'}¥${Number(r.amount || 0).toFixed(2)}
            </div>
        </div>
    `).join('');
}
...
function parseQuickAccountingInput(text) {
    ...
    const incomeKeywords = ['收入', '工资', '奖金', '理财', '收益', '盈利', '退款', '报销'];
